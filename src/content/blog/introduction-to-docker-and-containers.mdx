...
**How is it isolated?** The isolation of containers is provided by a Linux feature called *namespaces*<Sidenote>Linux namespaces were introduced incrementally starting around 2002 (mount namespaces) through 2013 (user namespaces) — containers as a concept only became practical once enough of these namespace types existed to isolate a full process tree.</Sidenote>. Namespaces partitions kernel resources such that one set of processes sees one set of resources while another set of processes sees a different set of resources. They give the group of running processes an isolated view of the kernel. For example,
...
Another feature of the **kernel** is *control groups* (a.k.a `cgroups`) which monitors, limits, accounts for, and isolates the resource usage of a collection of processes (also known as containers)<Sidenote>cgroups were originally developed at Google (under the name "process containers") and merged into the Linux kernel in 2007, years before Docker made the combination of cgroups and namespaces mainstream.</Sidenote>
...
An early implementation of container technology was added to **FreeBSD** in **2001**<Sidenote>That feature was FreeBSD Jails, which let administrators partition a system into several independent environments — conceptually similar to containers, though built on a very different kernel from Linux's namespaces and cgroups.</Sidenote>. Whereas Docker debuted to the public in **Santa Clara at PyCon in 2013** and was made open source in **March 2013**. The tooling for using Linux Containers was really lacking and that's where docker comes into play. Basically, **docker is tooling to manage containers.**
