...
// Deterministic per-post seed so each card's hash ring is subtly unique.
...
// with node placement rotated by the post's slug hash. Satori can't lay out
...
// dependency for a single template.
...
function card(post: {
  ...
  return el(
    ...
    el(
      ...
      mono('ARNAB SEN · WRITING', { fontWeight: 500, color: INK }),
      mono(`${dateLabel} · ${post.minutes} MIN`),
    ),
    // Title + rule + dek
    el(
      'div',
    ...
    el(
      ...
      el('img', { src: hashRingDataUri(hashSlug(post.slug)), width: 130, height: 130 }),
    ),
  );
}
...
export async function getStaticPaths() {
  ...
  return posts.map((post) => ({
    ...
    props: {
      ...
      date: post.data.date,
      tags: post.data.tags,
      minutes: readingTime(post.body ?? '', post.data.readingTimeOverride),
      slug: post.id,
    },
  }));
}
...
export const GET: APIRoute = async ({ props }) => {
  const svg = await satori(card(props as Parameters<typeof card>[0]), {
    width: WIDTH,
    height: HEIGHT,
